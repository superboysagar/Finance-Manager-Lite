...
const Modal = { 
    ...
    toggleScreen(index = null, confirm = null) {
        document.querySelector('.modal-overlay-confirm').classList.toggle('active');
    
        if (index !== null) {
            this.pendingDeleteIndex = index; // Store the transaction index
        }
    
        if (confirm === 'yes' && this.pendingDeleteIndex !== null) {
            Transaction.remove(this.pendingDeleteIndex);
            this.pendingDeleteIndex = null; // Reset after deleting
        }
    },
...
const Transaction = {
    ...
    remove(index) {
        ...
        Storage.set(Transaction.all);  // Update local storage
        App.reload();  // Refresh UI
    },
...
const DOM = {
    ...
    editTransaction(index) {
        ...
        titleConteiner.innerHTML = `Edit Transaction`
        indexItem = index
    },
